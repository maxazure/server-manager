...
# Python project specific commands
...
# Run database migrations if manage.py exists (Django project)
if [ -f "manage.py" ]; then
    log "Running database migrations..."
    if python3 manage.py migrate 2>&1 | tee -a "$LOG_FILE"; then
        log "Database migrations completed successfully"
    else
        log_and_update_db "failed" "Database migrations failed"
        exit 1
    fi
fi
...
if [ -f "manage.py" ]; then
    ...
    if python3 manage.py collectstatic --noinput 2>&1 | tee -a "$LOG_FILE"; then
        ...
        log_and_update_db "failed" "Static files collection failed"
        exit 1
    fi
fi
